...
#[cfg(test)]
use features::agent::MAX_AUDIT_ENTRIES;
use features::{
    agent::{AgentAuditLedger, AgentPatchPreviewState, AgentProposalState, preview_agent_summary},
    blocks::CommandBlockObserver,
};
use layout_projection::{
    collect_all_rows, full_frame_damage, max_scrollback_offset, pane_chrome_rects,
    pane_content_surface, pane_status_surface, pane_terminal_size, remap_cursor, remap_selection,
    scratch_surface, scratch_terminal_size, viewport_to_terminal_position, visible_row_range,
};
...
#[cfg(test)]
use noctrail_render::ChromeLayer;
use noctrail_render::{PaneBorderStyle, RenderBackend, RenderInput, RenderPlan, RenderRect, Rgba};
use noctrail_runtime::{OutputDrain, PaneId, PaneRuntime};
use noctrail_term::{
    DamageSet, MouseTrackingMode, Position, Selection, SelectionMode, ShellIntegrationEvent,
    TerminalSnapshot, TerminalState,
};
use thiserror::Error;

pub use features::agent::{
    AgentAuditEntry, AgentAuditKind, AgentContextBlock, AgentContextPreview,
};
pub use features::blocks::{CommandBlock, StructuredOutputKind, StructuredOutputLens};
pub use status::PaneStatusLine;
use status::{format_exit_status, selection_line_ending};
