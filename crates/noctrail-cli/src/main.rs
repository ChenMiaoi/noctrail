...
use noctrail_term::{
    ...
    Selection, SelectionMode, ShellIntegrationEvent, Style, TerminalSnapshot, TerminalState,
};
...
fn print_shell_hook(target: &str) -> Result<(), String> {
    let spec = select_hook_spec(target)?;
    print!("{}", (spec.render_script)());
    Ok(())
}

fn run_hook_smoke(filters: &[String]) -> Result<(), String> {
...
fn run_hook_probe(probe: &HookProbe) -> Result<ObservedHookCapabilities, String> {
    ...
    let observed = ObservedHookCapabilities {
        prompt: events
            .iter()
...
fn render_pwsh_hook() -> String {
    r#"function global:__NoctrailEmit([string]$Payload) {
    [Console]::Out.Write("`e]1337;Noctrail;$Payload`a")
}
