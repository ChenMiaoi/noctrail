...
当前 Noctrail 的渲染后端策略是：

- Linux / Windows 走 **OpenGL**
- macOS 走 **Metal**

也就是说：

- `RenderPlan` 负责提供要画什么
- `GUI` / 渲染层负责把这些 glyph、selection、cursor 交给对应平台的 GPU 后端
- 同时保留软件 fallback，避免单一后端把桌面可用性卡死

macOS 这里不应继续押 OpenGL，因为 Apple 官方文档已经把 OpenGL 标成 deprecated，推荐使用 Metal / MetalKit。

可参考：

- [Apple: About OpenGL for OS X](https://developer.apple.com/library/archive/documentation/GraphicsImaging/Conceptual/OpenGL-MacProgGuide/)
- [Apple: Updating an Application to Support the OpenGL 3.2 Core Specification](https://developer.apple.com/library/archive/documentation/GraphicsImaging/Conceptual/OpenGL-MacProgGuide/UpdatinganApplicationtoSupportOpenGL3/UpdatinganApplicationtoSupportOpenGL3.html)
