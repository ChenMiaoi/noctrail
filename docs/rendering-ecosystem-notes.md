...
当前 Noctrail 更适合采用的渲染策略是：

- Rust 侧用 **wgpu** 做统一图形层
- macOS 底层走 **Metal**
- Windows 底层走 **DirectX 12**
- Linux 底层优先走 **Vulkan**
- 必要时保留 software fallback 退化入口，避免单一 GPU 后端把桌面可用性卡死

也就是说：

- `RenderPlan` 负责提供要画什么
- `GUI` / 渲染层负责把这些 glyph、selection、cursor 交给 wgpu
- wgpu 再根据平台选择最合适的现代图形后端
- 如果 GPU 后端初始化失败，必须还能回退到软件渲染路径，而不是直接把应用卡死

这样比“全局固定 OpenGL”更符合你说的“每个平台用各自最新的、Rust 最兼容的”。

可参考：

- [wgpu 首页](https://wgpu.rs/)
- [wgpu backends 文档](https://wgpu.rs/doc/wgpu/struct.Backends.html)
